define(['app'], function(app) {
  app.provider(
    ...
    function() {    	
    	  ...
    	  var postLogoutState = 'app.login';
    	  var postLoginState = 'app.company';
    	...
    	*/    
    	    var setLoginData = function(user) {
    	        if (!user) {
    	    ...
    	    var getLoginData = function () {
    	    	var userData = sessionStorage.getItem('userLoggedIn');
    			var user = null;
    	    	if (userData){
    	    		user = JSON.parse(userData);
    	    		wrappedService.loginHandler(user);
    	            wrappedService.doneLoading = false;
    	        } else {
    	            wrappedService.userRole = userRoles.public;
    	            wrappedService.isLogged = false;
    	            wrappedService.isLoggedAsAdmin = false;
    	      ...
    	      loginHandler: function (user, status, headers, config) {
    	        ...
    	        wrappedService.isLoggedAsAdmin = false;
    	      	if(!user.id) {
    	            ...
    	            wrappedService.isLoggedAsAdmin = true;
    	    	}
    	        // setup 
    	        setLoginData(user);
    	        // update user
